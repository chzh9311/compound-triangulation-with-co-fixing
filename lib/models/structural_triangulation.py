...
def structural_triangulation_torch(n_cams, human_tree, poses_2d, confidences, lengths, projections, n_step):
    ...
    return stri_from_opt(D, m, G, lengths, bs, Nj, 0, n_step, device)


def stri_from_opt(D, m, G, lengths, bs, Nj, root_idx, n_step, device):
    Trs = torch.eye(3*Nj, device=device).float()

    # move root to the forefront
    if root_idx != 0:
        Trs[:3, :3] = 0
        Trs[3*root_idx:3*(root_idx+1), 3*root_idx:3*(root_idx+1)] = 0
        Trs[:3, 3*root_idx:3*(root_idx+1)] = torch.eye(3)
        Trs[3*root_idx:3*(root_idx+1), :3] = torch.eye(3)
    ...
    Irow = torch.eye(3, device=device).repeat((1, Nj)).unsqueeze(0)
    TrLam = Irow @ D @ Irow.transpose(1, 2)
    G_rest = G[:, :, 3:] 
    Q = - torch.linalg.solve(TrLam, Irow @ D @ G_rest)
    p = torch.linalg.solve(TrLam, Irow @ m)

    Eb = torch.eye(Nj * 3, device=device).unsqueeze(0).float()
    Eb = Eb[:, :, 3:]
    Ex = torch.zeros((Nj*3, 3), device=device).unsqueeze(0).float()
    Ex[:, :3, :] = torch.eye(3)

    A = (Eb + Ex @ Q).transpose(-1, -2) @ G.transpose(-1, -2) @ D @ G @ (Eb + Ex @ Q)
    beta = (Eb + Ex @ Q).transpose(-1, -2) @ G.transpose(-1, -2) @ (D @ G @ Ex @ p + m)
    ...
    max_err = torch.max(torch.abs(start_len.squeeze() - lengths.squeeze())).item()
    # if max_err > 40:
    #     n_step = 1
    for i in range(n_step):
        start_len = torch.norm(b.view(bs, -1, 3, 1), dim=2)
        target_len = (start_len * (n_step - i - 1) + lengths.unsqueeze(-1)) / (n_step - i)
        # target_len = (start_len*(n_step - i - 1) + lengths.unsqueeze(-1) * (i+1)) / n_step
        Db = block_diag_batch(b.view(bs, -1, 3, 1))
        tmp_inv = torch.linalg.inv(Db.transpose(-1, -2) @ Inv @ Db)
